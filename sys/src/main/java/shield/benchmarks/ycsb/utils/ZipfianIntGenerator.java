/**
 * Copyright (c) 2010-2016 Yahoo! Inc., 2017 YCSB contributors. All rights reserved. <p> Licensed
 ...
 * language governing permissions and limitations under the License. See accompanying LICENSE file.
 */
 ...
 */
public class ZipfianIntGenerator extends IntGenerator {
   ...
   */
  public ZipfianIntGenerator(int items) {
    this(0, items - 1);
  }
   ...
   */
  public ZipfianIntGenerator(int min, int max) {
   ...
   */
  public ZipfianIntGenerator(int items, double zipfianconstant) {
   ...
   * @param zipfianconstant The zipfian constant to use.
   */
  public ZipfianIntGenerator(int min, int max, double zipfianconstant) {
  ...
  public ZipfianIntGenerator(int min, int max, double zipfianconstant,
    ...
    countforzeta = items;
    eta =
        (1 - Math.pow(2.0 / items, 1 - theta)) / (1 - zeta2theta / this.zetan);
   ...
   * items. Use the zipfian constant thetaVal. Remember the new value of n so
  ...
  int nextInt(int itemcount) {
    ...
    if (itemcount != countforzeta) {
      ...
      synchronized (this) {
        if (itemcount > countforzeta) {
          ...
          // e.g. if you decrease the number of items,
          // then just subtract the zeta sequence terms for the items
          // that went away. This would be faster than
    ...
    int ret = (int) (base + ((itemcount) * Math.pow(eta * u - eta + 1, alpha)));
    setLastValue((int) ret);
    return ret;
  }
