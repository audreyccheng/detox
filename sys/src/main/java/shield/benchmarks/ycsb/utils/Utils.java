/**
 * Copyright (c) 2010 Yahoo! Inc., 2016 YCSB contributors. All rights reserved. <p> Licensed under
...
public final class Utils {
  ...
  private static long RAND_SEED = 0;
  private static boolean RAND_SEEDED = false;
  ...
  /**
   * Parses the byte array into a double. The byte array must be at least 8
   * bytes long and have been encoded using {@link #doubleToBytes}. If the array
   * is longer than 8 bytes, only the first 8 bytes are parsed.
   *
   ...
   * count can be in any state. For a more accurate count we could use
   * {@link Thread.getAllStackTraces().size()} but that freezes the JVM and
   * incurs a high overhead.
   *
   * @return An estimated thread count, good for showing the thread count over
   *         time.
   */
  ...
  /**
   * @return The current system load average if supported by the JDK. If it's
   *         not supported, the value will be negative.
   */
  ...
  /**
   * @return The total number of garbage collections executed for all memory
   *         pools.
   */
  ...
  /**
   * Returns a map of garbage collectors and their stats. The first object in
   * the array is the total count since JVM start and the second is the total
   * time (ms) since JVM start. If a garbage collectors does not support the
   * collector MXBean, then it will not be represented in the map.
   *
   * @return A non-null map of garbage collectors and their metrics. The map may
   *         be empty.
   */
