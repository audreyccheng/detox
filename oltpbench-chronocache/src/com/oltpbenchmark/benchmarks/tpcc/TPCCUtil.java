...
public class TPCCUtil {
	...
	public static Customer newCustomerFromResults(Map<String, Object> rs) throws SQLException {
		if (rs.get("c_first") == null) {
			throw new SQLException("things broke");
		}
		Customer c = new Customer();
		// TODO: Use column indices: probably faster?
		c.c_first = rs.get("c_first").toString();
		c.c_middle = rs.get("c_middle").toString();
		c.c_street_1 = rs.get("c_street_1").toString();
		c.c_street_2 = rs.get("c_street_2").toString();
		c.c_city = rs.get("c_city").toString();
		c.c_state = rs.get("c_state").toString();
		c.c_zip = rs.get("c_zip").toString();
		c.c_phone = rs.get("c_phone").toString();
		c.c_credit = rs.get("c_credit").toString();
		c.c_credit_lim = (float) Double.parseDouble(rs.get("c_credit_lim").toString());
		c.c_discount = (float) Double.parseDouble(rs.get("c_discount").toString());
		c.c_balance = (float) Double.parseDouble(rs.get("c_balance").toString());
		c.c_ytd_payment = (float) Double.parseDouble(rs.get("c_ytd_payment").toString());
		c.c_payment_cnt = (int) Double.parseDouble(rs.get("c_payment_cnt").toString());
		try {
			c.c_since = new Timestamp(Long.parseLong(rs.get("c_since").toString()));
		} catch (java.lang.NumberFormatException e) {
			c.c_since = new Timestamp(Date.parse(rs.get("c_since").toString()));
		}
		return c;
	}
